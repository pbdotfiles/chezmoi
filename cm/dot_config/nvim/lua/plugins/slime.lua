...
return {
	...
	},
	{
		...
		config = function()
			-- Remove slime 'ic' and 'ac' mappings that would otherwise override "visual selection around class" from mini.ai
			for _, mode in ipairs({ "o", "v" }) do
				pcall(vim.keymap.del, mode, "ic")
				pcall(vim.keymap.del, mode, "ac")
			end
		end,
		keys = {
			{
